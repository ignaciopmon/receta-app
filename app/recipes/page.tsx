...
export default function RecipesPage() {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === "visible") fetchData(true) // Silent fetch al volver a la pestaña
    }
  ...
  // EL CAMBIO IMPORTANTE ESTÁ AQUÍ (silent = false)
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (recipe) =>
          ...
          recipe.tags?.some((tag) => tag.toLowerCase().includes(query)), // EL BUSCADOR YA USA LAS ETIQUETAS
      )
    }
            ...
            <div className="grid gap-4 sm:gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {filteredRecipes.map((recipe) => (
                <RecipeCard
                  ...
                  link={recipe.link}
                  onUpdate={() => fetchData(true)} // AHORA ES UN UPDATE SILENCIOSO
                  category={recipe.category}
                  difficulty={recipe.difficulty}
                  isFavorite={recipe.is_favorite}
                  rating={recipe.rating}
                  tags={recipe.tags} // PASAMOS LAS ETIQUETAS AQUÍ
                />
              ))}
            </div>
          ) : (
