...
// 1. EXTRAEMOS LA LÓGICA A UN COMPONENTE HIJO PARA PODER USAR USE_SEARCH_PARAMS
function RecipesBrowser() {
  ...
  // INICIALIZAMOS LOS ESTADOS LEYENDO LA URL
  ...
  // EFECTO MAGICO: SINCRONIZAR ESTADOS CON LA URL
  useEffect(() => {
    ...
    // Solo empuja a la URL si los filtros son distintos de los actuales
    if (searchString !== currentString) {
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase
        .from("recipes").select("*").eq("user_id", user.id).is("deleted_at", null).order("created_at", { ascending: false })
      ...
      if (!silent) setTimeout(() => setIsLoading(false), 300)
    }
  }

  const checkWelcomeModal = async (userId: string) => {
    try {
      const { data: prefsData } = await supabase.from("user_preferences").select("has_seen_welcome_modal").eq("user_id", userId).single()
    ...
    } catch (welcomeError) { console.error("Error checking for welcome modal:", welcomeError) }
  }
      ...
      <main className="flex-1">
        <div className="container mx-auto py-12 px-4 max-w-7xl">
          ...
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            <RecipeCardSkeleton /><RecipeCardSkeleton /><RecipeCardSkeleton /><RecipeCardSkeleton />
          </div>
        </div>
      </main>
    )
    ...
    return (
      <main className="flex-1 flex items-center justify-center">
        <div className="text-center p-8 max-w-md">
          <div className="bg-destructive/10 text-destructive p-4 rounded-full w-fit mx-auto mb-4"><CookingPot className="h-8 w-8" /></div>
  ...
  return (
    <main className="flex-1 w-full pb-20">
      ...
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {recipes.length === 0 ? (
          ...
          <Empty className="py-16 border-none bg-transparent shadow-none">
            ...
            <EmptyDescription className="max-w-md">Try adjusting your filters or search terms to find what you're looking for.{!showComponents && " (Hidden components are not included in search)"}</EmptyDescription>
            <div className="flex flex-col sm:flex-row gap-3 mt-6">
              {/* Al limpiar los filtros, la URL se limpiará sola mágicamente */}
      ...
      {showWelcomeModal && <WelcomeModal userId={welcomeUserId} username={welcomeUsername} onClose={() => setShowWelcomeModal(false)} />}
    </main>
  )
}

// 2. EXPORTAMOS LA PÁGINA ENVUELTA EN SUSPENSE (REQUISITO DE NEXT.JS)
