...
function StarRating({ rating }: { rating: number | null }) {
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[...Array(5)].map((_, i) => (
        <Star key={i} className={cn("h-3 w-3", i < rating ? "text-yellow-500 fill-yellow-500" : "text-muted-foreground/30")} />
      ))}
    </div>
  )
}
...
export function RecipeCard({
  id, name, ingredients, steps, imageUrl, link, onUpdate, category, difficulty, isFavorite, rating, tags,
}: RecipeCardProps) {
  ...
  const handleToggleFavorite = async (e: React.MouseEvent) => {
    ...
    // OPTIMISTIC UI: Cambiamos la UI al instante
    const newFavoriteState = !currentFavorite
    setCurrentFavorite(newFavoriteState)
    
    try {
      const { error } = await supabase.from("recipes").update({ is_favorite: newFavoriteState }).eq("id", id)
      if (error) throw error
      // Si la BD se actualiza bien, avisamos silenciosamente al padre
      if (onUpdate) onUpdate()
    } catch (error) {
      console.error("Error updating favorite:", error)
      // Si falla en la BD, revertimos la estrella al estado anterior
      setCurrentFavorite(!newFavoriteState)
    }
  }
    ...
    <Link href={`/recipes/${id}`} className="block h-full group">
      <Card className="h-full overflow-hidden flex flex-col border border-border/40 bg-card transition-all duration-300 hover:shadow-xl hover:-translate-y-1 hover:border-border/80 p-0 gap-0 rounded-xl">
        ...
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-muted">
          ...
          <Button variant="ghost" size="icon" className={cn("absolute top-3 right-3 h-8 w-8 rounded-full backdrop-blur-sm transition-all", currentFavorite ? "bg-white/90 dark:bg-black/90 text-yellow-500 hover:text-yellow-600 hover:bg-white" : "bg-black/20 text-white hover:bg-white hover:text-yellow-500")} onClick={handleToggleFavorite}>
        ...
        <div className="flex flex-col flex-1 p-5">
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground/90 line-clamp-2 leading-relaxed italic mb-3">
            ...
            {tags && tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-auto pt-2 border-t border-border/20">
                {tags.slice(0, 3).map(tag => (
                  <span key={tag} className="text-[10px] font-medium text-muted-foreground bg-muted/50 px-2 py-0.5 rounded-full border border-border/50">
                    #{tag}
                  </span>
                ))}
                {tags.length > 3 && (
        ...
        <div className="p-4 pt-0 mt-auto h-12 flex items-end justify-between gap-2">
          <div className="flex gap-1 w-full justify-between transition-all duration-300 opacity-100 transform translate-y-0 md:opacity-0 md:translate-y-1 md:group-hover:opacity-100 md:group-hover:translate-y-0">
            <div className="flex gap-1">
              <Button asChild variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary" onClick={(e) => e.stopPropagation()}><Link href={`/recipes/edit/${id}`}><PenSquare className="h-4 w-4" /></Link></Button>
              {link && <Button asChild variant="ghost" size="icon" className="h-8 w-8 hover:bg-primary/10 hover:text-primary" onClick={(e) => e.stopPropagation()}><a href={link} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-4 w-4" /></a></Button>}
            </div>
            <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10" onClick={handleDelete} disabled={isDeleting}><Archive className="h-4 w-4" /></Button>
          </div>
        </div>
      </Card>
    </Link>
